...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(StringUtils.hasText(token) && result) {
            log.trace("토큰이 있고, 유효함을 인증");
            ...
            log.trace("sc  = {}", SecurityContextHolder.getContext());
        } else {
            log.trace("토큰이 없거나 유효하지 않음");
        }
        filterChain.doFilter(request, response);
        log.trace("doFilterInternal 끝");
    }
...
//    }
    ...
    public void verifyTokenAndGetAuthorities(String token) {
        ...
        try {
            ...
            // 권한 정보 확인
            String authorities = (String) claims.get("authorities");
            log.trace("토큰 권한 정보: {}", authorities);
        } catch (Exception e) {
            log.error("토큰 검증 및 권한 확인 중 오류 발생", e);
        }
        log.trace("토큰 검증 및 권한 확인 종료");
    }
}
